...
class ValidationLimits:
    ...
    MIN_INNER_DIAMETER = 60.0
    MAX_INNER_DIAMETER = 260.0

    MIN_OUTER_DIAMETER = 70.0
    MAX_OUTER_DIAMETER = 300.0

    MIN_FABRIC_THICKNESS = 0.0

    MIN_PUNCH_HEAD_LENGTH = 8

    MAX_PUNCH_DEPTH = 15.0

    MAX_IDLING_SPEED = 6000.0
    MIN_IDLING_SPEED = 100.0

    MAX_MOVE_SPEED = 2000.0
    MIN_MOVE_SPEED = 100.0

    MAX_ROTATE_SPEED = 2000.0
    MIN_ROTATE_SPEED = 100.0

    MAX_RANDOM_BORDER = 0.5
    MIN_RANDOM_BORDER = 0.0
...
base_description = '''
Приложение позволяет сгенерировать управляющую программу (G-код) для 
...
4) Указать название файла для сохранения G-кода (без расширения, только имя)
5) Выбрать папку для сохранения G-кода
6) Проверить информацию о параметрах, пусти сохранения и нажать кнопку генерации G-кода
7) При успешной генерации появится соответсвующее сообщение в нижней части приложения
7*) В случае ошибки отобразиться информация по возможным причинам и о вариантах исправления
'''

version_description = '''
Версия 2.0 от 2025/12/01

- Возможность использовать несколько рядов игл
- "Безопасная" установка нуля
- Визуализация точек пробития для центрального ряда
'''